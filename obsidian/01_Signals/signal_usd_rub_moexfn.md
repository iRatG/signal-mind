...
| Год | r | n | Статус |
|-----|---|---|--------|
| 2022–2023 (Discovery) | **+0.83** | 485 | ✅ очень сильный |
| 2024 (Validation) | **-0.47** | 252 | ❌ инверсия — сигнал НЕ прошёл holdout |
| 2025 (Live) | +0.40 | 253 | ⚠️ частичное восстановление |
...
## Открытые атаки
- [[attack_non_stationarity]] — holdout 2024: r=-0.47 (инверсия подтверждена количественно) ← **P0**
- [[attack_in_sample_overfitting]] — **сигнал НЕ прошёл holdout** Discovery r=0.83 → Validation r=-0.47 ← **P0 CRITICAL**
- [[attack_regime_conditionality]] — при USD/RUB > 90 корреляция инвертируется (Marathon 5 подтвердил)

## ⚠️ Вердикт holdout (2026-05-02)
**Сигнал признан нестационарным.** Нельзя использовать для торговли в текущем виде.
Discovery r=0.83 объясняется периодом 2022-2023 (жёсткие санкции, адаптация рынка).
2024: ЦБ поднял ставку до 21%, механизм сломался (банки под кредитным стрессом).
