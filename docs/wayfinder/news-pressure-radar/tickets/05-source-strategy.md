...
**Revisit condition checked 2026-07-31, when Ticket 07 closed — not triggered.** Ticket 07 confirmed all three delivery surfaces (Telegram channel, email, local web page) stay strictly private/Airat-only, including a domain/VPS-hosting idea Airat raised, which he confirmed was for uptime only, not publicity. This policy stands unchanged; re-check again only if/when the project ever actually publishes or redistributes to an outside audience.
