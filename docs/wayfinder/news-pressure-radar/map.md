...
- **Session resume point (2026-07-31, fourth update — Ticket 04 closed this session; verify current commit with `git log --oneline -5` before trusting anything below as pushed):** Tickets 01, 02, 03, 04, 05, and 10 are closed — see each ticket's `## Working Decision` + `## Grilling Transcript` for the full record, not just the one-line gists in `## Decisions So Far` above.
  - `docs/wayfinder/news-pressure-radar/idea-buffer.md` holds cross-ticket architectural ideas that surfaced mid-grill but don't belong to one ticket (registry-driven entities; stable module interfaces/role independence; as of this session, also "fix per the documented concept, never an ad-hoc patch"). Check it before assuming a principle lives only on the ticket that first mentioned it.
  - Ticket 04 (Report Prototype) closed this session — see `## Decisions So Far` below for the outcome. It produced a new project-wide mechanism (Product vs. Technical/Ops report mode split) and a concrete example artifact at `docs/wayfinder/news-pressure-radar/prototypes/04-daily-report-example.md`. It also reinforced Ticket 11's diagnostic-agent forward note and left a multi-locale/tone/chart forward note on Ticket 07.
  - **Next session should claim [Ticket 06 - Evaluation Harness](tickets/06-evaluation-harness.md) first** (formally unblocked, can now backtest against Ticket 10's closed coupling model and reference Ticket 04's closed report shape for what "success" should look like when rendered), **then [Ticket 11 - Agentic Delivery Process](tickets/11-agentic-delivery-process.md)** (has accumulated 5 forward notes across sessions now — parameter-registry hierarchy, diagnostic-agent role reinforced twice, cost/model split, Auditor separation, and the new fix-per-spec principle — worth reading all of them before grilling starts). See `## Current Frontier` below for the full reasoning behind this order.
...
Priority order refreshed 2026-07-31 after Ticket 04 closed:

1. [Ticket 06 - Evaluation Harness](tickets/06-evaluation-harness.md) — **resume here next session.** Formally unblocked (Signal Ontology + Quality Gate Contract + Source Strategy all closed); its forward note's "reference correlation" work can now reference both Ticket 10's closed market-first coupling model and Ticket 04's closed report shape.
2. [Ticket 07 - Delivery Policy](tickets/07-delivery-policy.md) — now unblocked (Product Contract, Quality Gate Contract, and Report Prototype all closed). Carries two forward notes from Ticket 04's grilling: the Editor/multi-channel rendering scope (tone, color, charts, Telegram-vs-email split) and a multi-locale/multi-audience rendering question, both explicitly deferred here rather than decided in Ticket 04.
3. [Ticket 11 - Agentic Delivery Process](tickets/11-agentic-delivery-process.md) — meta-process ticket; nothing else in the graph blocks on it. Has accumulated 5 forward notes now (parameter-registry hierarchy, diagnostic-agent role reinforced twice across Tickets 03 and 04, cost/model split, Auditor separation, and the new fix-per-spec-not-ad-hoc-patch principle from Ticket 04).
4. [Ticket 12 - Historical Cluster Calibration Study](tickets/12-historical-cluster-calibration.md) — runs whenever, in parallel with any of the above; no live grilling needed. Scope covers both the English `hf_news.db` archive and the new 14-source RU set; does not gate Ticket 03/04/05/06/10's numbers today, only Ticket 08 eventually.

## Blocked Tickets

- [Ticket 08 - OpenSpec Bridge](tickets/08-openspec-bridge.md) depends on Product Contract, Signal Ontology, Quality Gate Contract, Report Prototype, Delivery Policy (07 — still open), and Ticket 12 - Historical Cluster Calibration Study (numeric sign-off only — Tickets 02/03/04/05/10 themselves are not blocked by it).
...
7. **← Next.** Resolve [Ticket 06 - Evaluation Harness](tickets/06-evaluation-harness.md) now that Signal Ontology, Quality Gate Contract, Source Strategy, Market Coupling Model, and Report Prototype are all closed — a coupling model and a report shape now both exist to backtest/render against.
8. Resolve [Ticket 07 - Delivery Policy](tickets/07-delivery-policy.md), now unblocked — carries Ticket 04's Editor/multi-channel and multi-locale forward notes.
9. Resolve [Ticket 11 - Agentic Delivery Process](tickets/11-agentic-delivery-process.md) so the project teaches the workflow, not only the market idea — nothing else blocks on this, so it can slot in whenever convenient.
